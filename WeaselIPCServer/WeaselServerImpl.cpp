...
DWORD ServerImpl::OnSendString(WEASEL_IPC_COMMAND uMsg,
                               DWORD wParam,
                               DWORD lParam) {
  if (m_pRequestHandler) {
    // 从管道缓冲区读取文本
    std::wstring text(reinterpret_cast<LPWSTR>(channel->ReceiveBuffer()));
    m_pRequestHandler->CommitText(text, lParam);
  }
  return 0;
}
...
void ServerImpl::HandlePipeMessage(PipeMessage pipe_msg, _Resp resp) {
  ...
  PIPE_MSG_HANDLE(WEASEL_IPC_SEND_STRING, OnSendString);
