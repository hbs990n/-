...
bool ClientImpl::SendString(const std::wstring& text) {
  if (!_Active())
    return false;

  // 将文本写入管道缓冲区
  channel << text.c_str();
  LRESULT ret = _SendMessage(WEASEL_IPC_SEND_STRING, 0, session_id);
  return ret != 0;
}
...
bool Client::SendString(const std::wstring& text) {
  return m_pImpl->SendString(text);
}
